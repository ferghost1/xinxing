...
if($number)
{
    if($s) {
        $page = new apps_libs_Page([
            "table" => "detailacc,acc",
            "where" => "detailacc.idacc=acc.id and acc.type='user' and (acc.user LIKE '" . $s . "%' or detailacc.name LIKE '" . $s . "%') ORDER BY acc.user ASC",
            "col" => [
                "ID" => "acc.id",
                "Tài Khoản" => "acc.user",
                "Tên" => "detailacc.name",
    ...
    else
    {
        $page = new apps_libs_Page([
            "table" => "detailacc,acc",
            "where" => "detailacc.idacc=acc.id and acc.type='user' ORDER BY acc.user ASC",
            "col" => [
                "ID"=>"acc.id",
                "Tài Khoản" => "acc.user",
                "Tên" => "detailacc.name",
