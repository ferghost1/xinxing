...
global $db,$time,$username,$revenue;
      ...
      if(!empty($child)){
        foreach ($child as $key => $value) {
          foreach($value as $k1 => $v1){
            $total += $v1['data']['money']*$v1['data']['percent']/100;
          }
        }
      }
...
function create_agency_table($data){
  ...
  $uti=new apps_libs_Utilities();
    ...
    <div class="col-md-12" style="margin-bottom: 30px">
      ...
      <span style="font-size: 18px">
        Tổng hoa hồng: <bold style="color:red;font-size: 18px"><?= $uti->EditNumber($sum_money*$revenue->agency_rate/100) ?></bold>
      </span>
       ...
       foreach($data as $k => $v):
        ...
        ?>
          <tr>
            <td><?= $i ?></td>
            <td><?= $v['username'] ?></td>
            <td><?= number_format($v['money']) ?></td>
            <td><?=$revenue->agency_rate?>%</td>
            <td><?= $uti->EditNumber($v['money']*$revenue->agency_rate/100)?></td>
            <td><?=$username?></td>
