...
if ($number) {
    if ($s) {
        $page = new apps_libs_Page([
            "table" => "detailacc,acc,listproduct,historyproduct",
            "where" => "(acc.type='user' and acc.user LIKE '" . $s . "%' or detailacc.name LIKE '" . $s . "%') and detailacc.idacc=acc.id and acc.type!='admin' and acc.id= historyproduct.idacc and historyproduct.id = listproduct.idhp and MONTH(historyproduct.timecreate)=".$time['month']." and YEAR(historyproduct.timecreate)=".$time['year']." GROUP BY acc.user ORDER BY SUM(listproduct.price * listproduct.quantity) DESC",
            "col" => [
                "ID" => "acc.id",
                "Tài Khoản" => "acc.user",
                "Tên" => "detailacc.name",
                "Doanh Thu Phát Sinh" => "SUM(listproduct.price * listproduct.quantity) as total",
            ],
            "function" => [
                "in" => "user",
                "out" => "id",
        ...
        $page = new apps_libs_Page([
            "table" => "detailacc,acc,listproduct,historyproduct",
            "where" => "acc.type='user' and detailacc.idacc=acc.id and acc.type!='admin' and acc.id= historyproduct.idacc and historyproduct.id = listproduct.idhp and MONTH(historyproduct.timecreate)=".$time['month']." and YEAR(historyproduct.timecreate)=".$time['year']." GROUP BY acc.user  ORDER BY SUM(listproduct.price * listproduct.quantity) DESC",
            "col" => [
                "ID" => "acc.id",
                "Tài Khoản" => "acc.user",
                "Tên" => "detailacc.name",
